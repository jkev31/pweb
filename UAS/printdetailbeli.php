<?php
require_once __DIR__ . '/fpdf19/fpdf.php';$raw    = json_decode($_GET['data'] ?? '{}', true) ?: [];
$header = $raw['header'] ?? [];
$items  = $raw['items']  ?? [];
$footer = $raw['footer'] ?? [];
...
$pdf->Cell(0, 10, 'Detail Pembelian', 0, 1, 'C');
$pdf->Ln(2);
...
$pdf->Cell(0, 8, $header['kodepb'] ?? '', 0, 1);
...
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(40, 8, 'Supplier', 0, 0);
$pdf->Cell(5, 8, ':', 0, 0);
$pdf->SetFont('Arial', '', 11);
$pdf->Cell(0, 8, $header['nama_supplier'] ?? '', 0, 1);
...
$headers = ['No', 'Nama', 'Satuan', 'Harga Beli', 'Qty', 'Subtotal'];
...
foreach ($items as $idx => $item) {
    ...
    $pdf->Cell($colW[3], 8, 'Rp ' . number_format((float)preg_replace('/[^0-9]/', '', $item['hbeli'] ?? '0'), 0, ',', '.'), 1, 0, 'R');
...
$diskonText ='Rp ' . ($footer['diskon_nominal'] ?? '0');
